...
public class EnumConstant{
	...
	public static enum SmsBuss{
        ...
        设置快捷支付密码效验码("fastPayPassword",70),
        修改快捷支付密码效验码("upFastPayPassword",71)
        ;
		...
		public static SmsBuss getSmsBuss(String bus) {
			...
			}else if(bus.equals("login")){
				return SmsBuss.登录校验码;
			...
			}else if(bus.equals("fastPayPassword")){
				return SmsBuss.设置快捷支付密码效验码;
			}else if(bus.equals("upFastPayPassword")){
				return SmsBuss.修改快捷支付密码效验码;
			}else{
				return SmsBuss.空;
			}
		}
