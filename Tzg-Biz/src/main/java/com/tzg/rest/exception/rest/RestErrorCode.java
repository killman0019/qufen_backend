...
public enum RestErrorCode {
	...
	FINAICAL_RECORD_NOT_EXIST(9991, "资金流水记录不存在"), MISSING_ARG_PROJID(9990, "参数缺失项目ID"), MISSING_ARG_MESSAGEID(9989, "缺失/非法消息记录ID"), MISSING_ARG_POSTID(9988,
			"缺失/非法帖子记录ID"), MISSING_ARG_FOLLOWTYPE(9987, "非法关注类型"), MISSING_ARG_FOLLOWID(9986, "非法关注ID"), POST_NOT_EXIST(9985, "帖子不存在"), POST_ID_BLANK(9984,
			"帖子id不能为空"), MISSING_ARG_COMMENTSID(9983, "评论id不能为空"), EVA_NOT_EXIST(9982, "评测不存在"),
	...
	USER_ICON_NICKNAME_NULL(11022, "用户头像和昵称不能同时为空"), USER_NOT_REAL_AUTH(11023, "您还未实名认证,请先进行实名认证,谢谢"), USER_NOT_LOGIN(11024, "用户未登录, 请重新登录"), USER_ICON_DATA_EMPTY(
			11025, "头像不能为空"), USER_SEX_WRONG(11026, "用户性别传参非法"), USER_ID_BLANK(11027, "用户id不能为空"), USER_CARD_POSITIVE_OF_CARD(11028, "用户手持身份证正面照不能为空"),

	USER_CARD_REVERSE_OF_CARD(11029, "用户手持身份证反面照不能为空"), USER_IDCARD_IS_FALSE(11030, "身份证号码不正确"), USER_IDCARD_SUBMITED(11031, "身份证号码已被实名验证"),
	...
	RED_ENVELOPE_code_ERROR(13039, "兑换码错误，请重试"), DID_NOT_SET_FAST_PAY_PASSWORD(13039, "请先设置手机支付密码"),

	BANK_CARD_NOT_EXIST(13040, "银行卡号不存在，请重试"),

	MISSING_ARG_BBID(13041, "绑卡记录ID缺失"), MISSING_ARG_CHANNEL(13042, "iChannelType缺失"), MISSING_ARG_IMGTYPE(13043, "图片类型缺失"), WRONG_IMGTYPE(13044, "错误的图片类型"),
	/** ------------------ 标相关错误码以15开头 ----------------- */

	SUBJECT_NOT_EXIST(15001, "标的不存在"), MISS_SUBJECT_ID(15015, "缺失标的ID"),
	...
	/**********************内容类错误********************************/
	SHARE_POST_FAIL(200001, "分享内容失败,不能领取分享奖励");
