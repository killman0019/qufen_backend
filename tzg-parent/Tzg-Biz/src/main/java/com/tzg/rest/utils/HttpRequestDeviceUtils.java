...
public class HttpRequestDeviceUtils {
    ...
    private static String[] deviceHeaders=new String[]{
    "Windows 98",
    "Windows ME",
    "Windows 2000",
    "Windows XP",
...
//    "Macintosh"
    ...
    */
    ...
    /**app*/
    public static Integer getTerminalType(HttpServletRequest request) {
        String deviceType = HttpRequestDeviceUtils.getAppDeviceType(request);
        Integer tType = 3;
        if(deviceType!= null && deviceType.equals("Android")){
            tType = 4;
        }
        if(deviceType!= null && (deviceType.equals("iPhone")|| deviceType.equals("iPad"))){
            tType = 3;
        }
        return tType;
    }
    ...
    /**手机端用*/
    private static String[] appDeviceHeaders=new String[]{
    "iPhone",
    "iPad",
    "iPod",
    "Android",
    ...
    /**手机端用*/
    public static String getAppDeviceType(HttpServletRequest request){
        String userAgent = request.getHeader("user-agent");
        for (int i = 0; userAgent!=null && !userAgent.trim().equals("") && i < appDeviceHeaders.length; i++) {
             if(userAgent.contains(appDeviceHeaders[i])){
                   return appDeviceHeaders[i];
             }
        }
        //default iPhone
        return "iPhone";
    ...
    /**手机端用*/
    public static String getAppDeviceType(HttpServletRequest request,String plat){
        String result = "iPhone"; //default iPhone
     ...
     * @return
     */
    public static RestRequestHead getUserAgent(HttpServletRequest httpServletRequest) {
    	RestRequestHead head = new RestRequestHead();
        String platform = httpServletRequest.getHeader("platform");
        //"1"--ios个人版,"2" 安卓,"3" ios企业版
        head.setPlatform(StringUtils.isNotEmpty(platform) ? Integer.valueOf(platform) : 2);
        head.setPhoneModel(httpServletRequest.getHeader("phoneModel"));
        head.setSystemVersion(httpServletRequest.getHeader("systemVersion"));
        head.setAppVersion(httpServletRequest.getHeader("appVersion"));
        head.setOperator(httpServletRequest.getHeader("operator"));
        head.setNetworkType(httpServletRequest.getHeader("networkType"));
        return head;
    }
