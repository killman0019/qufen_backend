...
public abstract class BaseController {
	 ...
	 * 直接输出二进制
	 */
	public void responseBin(HttpServletResponse response, byte[] buf, int off,
			int len) {
		try {
	...
	public void responseText(HttpServletResponse response, String text) {
		doPrint(response, text, "application/plain;charset=UTF-8");
	}

	public void constructBaseResponseEntity(HttpServletRequest request,
			HttpServletResponse response, BaseResponseEntity entity) {
		entity.setCode(0);
	...
	public void throwRestException(BaseResponseEntity bre, String msg,
			RestErrorCode re) {
	...
	public JSONObject getParamMapFromRequest(HttpServletRequest request)
			throws Exception {
	...
	public <T> T getParamMapFromRequest(HttpServletRequest request,
	...
	public String getTerminalDeviceType(HttpServletRequest request) {
		...
		if (null != deviceType
				&& (deviceType.equals("iPhone") || deviceType.equals("iPad"))) {
			tType = "ios";
		}
		return tType;
	}
	...
	public Integer getTerminalType(HttpServletRequest request) {
		...
		if (deviceType != null
				&& (deviceType.equalsIgnoreCase("iPhone") || deviceType
