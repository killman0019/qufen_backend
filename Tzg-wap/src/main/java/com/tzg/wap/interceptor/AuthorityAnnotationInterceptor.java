...
public class AuthorityAnnotationInterceptor extends HandlerInterceptorAdapter {
	...
	private boolean preparedSessionUser(HttpServletRequest request,
		...
		String currentURL = request.getRequestURI(); // 取得根目录所对应的绝对路径:相对路径
		...
		String suffix = StringUtils.lowerCase(StringUtils.substringAfterLast(
				currentURL, "."));
		...
		if (StringUtils.isNotBlank(surc)) {
			request.getSession().setAttribute(TzgConstant.SOURCE_STORE_KEY,
					surc);
			CookieUtil.addCookie(response, TzgConstant.SOURCE_STORE_KEY, surc,
					2592000); // 保存一个月
		}
		// app来源表示
		...
		if (loginAccount == null) {
			...
			if (StringUtils.isNotBlank(currentURL)
					...
					&& (currentURL.indexOf("/user") != -1 || currentURL
							.indexOf("/subject") != -1)) {
				String url = request.getRequestURI().substring(
						request.getContextPath().length());
				StringBuffer redirectURL = new StringBuffer(url);
				...
				// request.getSession().setAttribute("LOGIN_AFTER_REDIRECT_URL",
	...
	public void appLogin(String key) throws Exception {
		...
		try {
			...
			if (StringUtil.equals(sign, keys[1])) {
				String rediskey = SHAUtil.encode(keys[0]
						+ TzgConstant.LOGIN_SIGN_KEY)
						+ TzgConstant.TZG_REST;
	...
	private static final Log LOG = LogFactory
			.getLog(AuthorityAnnotationInterceptor.class);
