...
public class KFFRmiServiceImpl implements KFFRmiService {
	...
	@Override
	public KFFUserHomeResponse findUserHomeByUserId(@RequestParam(value = "loginUserId", required = false)Integer loginUserId,
	...
	public void savePraise(Integer userId, Integer postId) throws RestServiceException {
		...
		Post post = kffPostService.findById(postId); // 文章详情对象
		if(post == null){
		...
		if(praise == null){
		    ...
		    save.setBepraiseUserId(post.getCreateUserId()); // 添加被点赞人的id
		    save.setCreateTime(now);  // 添加点赞的时间
		    save.setPostId(postId);  // 添加点赞的帖子id
		    save.setPostType(post.getPostType());  // 添加点赞帖子的类型 1-评测 , 2-讨论 , 3-文章
		    save.setPraiseUserId(userId);	//  添加点赞人的id 
		    save.setProjectId(post.getProjectId());  // 添加点赞项目的id
		    save.setStatus(1);	// 添加点赞的状态  1-有效赞   0-取消点赞
		    save.setUpdateTime(now); // 添加修改时间
				...
				if(postType == 1) {
					...
					if (evaluation.getModelType()==1) {
						...
						if (praise.getPraiseType()==1) {
							...
							if(praise.getStatus()==1 && validPraise>0){
								...
								tokenrecords.setAmount(5.00000000f*createPUF); // 点赞奖励生成流水 
					...
					if (evaluation.getModelType()==2){
						...
						if (praise.getPraiseType()==1) {
							...
							if(praise.getStatus()==1 && validPraise>0){
								...
								if(kffPostService.findById(postId).getPraiseNum()==500) {
									tokenrecords.setFunctionDesc("用户赞赏");
									tokenrecords.setFunctionType(5); 
									tokenrecords.setAmount(50.00000000f * createPUF + meet); // 点赞奖励生成流水 
									tokenrecords.setUserId(createUserId);
									tokenrecords.setTradeType(1);
									tokenrecords.setRewardGrantType(1);
									tokenrecords.setCreateTime(new Date());
									...
									tokenrecords.setFunctionDesc("用户赞赏");
									tokenrecords.setFunctionType(5); 
									tokenrecords.setAmount(50.00000000f*createPUF); // 点赞奖励生成流水 
									tokenrecords.setUserId(createUserId);
									tokenrecords.setTradeType(1);
									tokenrecords.setRewardGrantType(1);
									tokenrecords.setCreateTime(new Date());
						...
						if (praise.getPraiseType()==2) {
							...
							tokenrecords.setFunctionDesc("用户赞赏");
							tokenrecords.setFunctionType(5); 
							tokenrecords.setAmount(2 * createPUF); // 点赞奖励生成流水 
							tokenrecords.setUserId(createUserId);
							tokenrecords.setTradeType(1);
							tokenrecords.setRewardGrantType(1);
							tokenrecords.setCreateTime(new Date());
					...
					if (evaluation.getModelType()==3){
						...
						tokenrecords.setFunctionDesc("用户赞赏");
						tokenrecords.setFunctionType(5); 
						tokenrecords.setAmount(20f * createPUF); // 点赞奖励生成流水 
						tokenrecords.setUserId(createUserId);
						tokenrecords.setTradeType(1);
						tokenrecords.setRewardGrantType(1);
						tokenrecords.setCreateTime(new Date());
				...
				if(postType == 2) {
					...
					if (praise.getPraiseType()==2) {
						...
						if(praise.getStatus() <= validPraise){
							...
							tokenrecords.setFunctionDesc("用户赞赏");
							tokenrecords.setFunctionType(5); 
							tokenrecords.setAmount(2f * createPUF); // 点赞奖励生成流水 
							tokenrecords.setUserId(createUserId);
							tokenrecords.setTradeType(1);
							tokenrecords.setRewardGrantType(1);
							tokenrecords.setCreateTime(new Date());
					...
					if (praise.getPraiseType()==1) {
						...
						if(praise.getStatus() <= validPraise){
							...
							if (praise.getPraiseType()==1) {
								...
								tokenrecords.setFunctionDesc("用户赞赏");
								tokenrecords.setFunctionType(5); 
								tokenrecords.setAmount(20*createPUF+meet); // 点赞奖励生成流水 
								tokenrecords.setUserId(createUserId);
								tokenrecords.setTradeType(1);
								tokenrecords.setRewardGrantType(1);
								tokenrecords.setCreateTime(new Date());
	...
	public List<CoinProperty> findCoinPropertyById(Integer userId, Date coinUnlockTime,Integer coinUnlockType) throws RestServiceException {
		...
		if( CollectionUtils.isNotEmpty(coinProperty)){
			...
			for(CoinProperty coinPropertys:coinProperty){
				...
				user.setKffCoinNum(kffCoinNum + coinUnlock);
